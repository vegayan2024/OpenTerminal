    ...
    <div>
      <div className="flex gap-2 p-1 flex-wrap items-center">
        <select value={sector} onChange={(e) => setSector(e.target.value)}>
          <option value="">全部行业板块</option>
          {sectors.map((s) => (
            <option key={s} value={s}>{s}</option>
          ))}
        </select>
        <input className="w-20" placeholder="涨跌幅下限%" value={changeMin} onChange={(e) => setChangeMin(e.target.value)} />
        <input className="w-28" placeholder="市值下限 (亿元)" value={marketCapMinB} onChange={(e) => setMarketCapMinB(e.target.value)} />
        <input className="w-28" placeholder="成交量下限 (万手)" value={volumeMinM} onChange={(e) => setVolumeMinM(e.target.value)} />
        <span className="dim ml-auto">{isLoading ? "正在检索…" : `共 ${data.length} 只标的`}</span>
      </div>
      {error && <div className="p-2 down">选股数据异常: {(error as Error).message}</div>}
